...
def get_from_wg_api(name, url):
    r = urllib.request.urlopen(
        url='https://api.worldofwarships.asia/{0}'.format(url))
    if r.status == 200:
        text = r.read()
...
def get_players(search):
    ...
    url = 'wows/account/list/?application_id={0}&search={1}'.format(
        wg_app_id, search)
    return get_from_wg_api(name, url)


def get_player_personal_data(account_id):
...
def get_player_clan_data(account_id):
...
def get_player_ship_stats(account_id):
